...
velocity_0 = Function(Vs, name="velocity_0")
...
class InitialConditions(UserExpression):
    ...
    def eval(self, values, x):
        values[0] = (t-1)*(t-2) + t*(t-2) + t*(t-1)  #values[0] = 0.1*self.t
        values[1] = 0.0
    def value_shape(self):
        return (2,)
    
U_init = InitialConditions(t=0, degree=1)
...
for n in range(1, num_steps + 1):
    ...
    U_init.t = n*dt
    velocity_0.interpolate(U_init)
    # step 4. calculate body force.
    force.vector()[:] = velocity_0.vector()[:]- velocity.vector()[:]/dt   #- 1000000*(disp.vector()[:] - disp0.vector()[:])
