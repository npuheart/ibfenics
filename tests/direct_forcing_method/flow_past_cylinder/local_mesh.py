...
def calculate_solid_mesh(n_mesh_solid):
    solid_mesh = Mesh()
    with XDMFFile(mesh_path + "cylinder_40.xdmf") as xdmf:
        xdmf.read(solid_mesh)
    return solid_mesh


solid_mesh = calculate_solid_mesh(n_mesh_solid)
bdry = MeshFunction("size_t", solid_mesh, mesh_path + "cylinder_40_boundaries.xml")
domains = MeshFunction("size_t", solid_mesh, mesh_path + "cylinder_40_domains.xml")
