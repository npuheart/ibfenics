...
for n in range(1, num_steps + 1):
    ...
    u1, p1 = navier_stokes_solver.solve(bcu, bcp) #解NS方程的u_(n+1),p_(n+1)
    navier_stokes_solver.update(u1, p1)
    # logger.info(f"kinematic_energy(u0) {kinematic_energy(u0)}")
    # step 2. interpolate velocity from fluid to solid  (u->U)
    ...
    advance_disp_be(disp, velocity, dt)  #更新固体的位移
    ib_interpolation.evaluate_current_points(disp._cpp_object)
