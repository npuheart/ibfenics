...
class TaylorHoodSolver:
    def __init__(self, u0, p0, f, dt=0.01, nu=0.01, u_boundary = None):
        ...
        F = inner((u-self.un)/k, v)*dx + inner(grad(self.un)*self.un, v)*dx + \
            nu * inner(grad(u), grad(v))*dx - div(v)*p * dx - inner(f, v)*dx
        F += q*div(u)*dx
        
        G = f
        F1 = - inner(grad(p), grad(q))*dx + inner(G, grad(q))*dx - nu*cross(N,self.un)*grad(q)*ds - inner(u_boundary,N)*q*ds
        F1 = - inner(grad(p), grad(q))*dx - inner(grad(self.un)*self.un, grad(q))*dx*dx
        
        F2 = inner(u,grad(v))*dx + inner(grad(u), grad(v))*dx -1.0/nu*inner(G - grad(p), grad(q))*dx
        F2 = inner(u,grad(v))*dx + inner(grad(u), grad(v))*dx +1.0/nu*inner(grad(self.un)*self.un + grad(p), grad(q))*dx
